// tests/edge_case_tests_fixed.rs
...
fn test_ring_buffer_wraparound() {
    ...
    for i in 0..capacity * 2 + 1 {
        ...
        if i % 2 == 1 {
            consumer.process_event_batch(|event, seq, _end_of_batch| {
...
fn test_consumer_dependency_chain() {
    ...
    for _ in 0..5 {
        // Consumer1 processes
        consumer1.process_event_batch(|event, _seq, _end_of_batch| {
            event.value *= 2; // Double the value
        });
        
        // Consumer2 processes (depends on consumer1)
        consumer2.process_event_batch(|event, _seq, _end_of_batch| {
            event.value += 10; // Add 10
        });
        
        // Consumer3 processes (depends on consumer2)
        consumer3.process_event_batch(|event, _seq, _end_of_batch| {
